	...
	$.fn.DataFilter = function(method, options) {
		...
		function initDataElement(config) {
		  ...
		  /* END Filters - Properties */
			...
			var createFn = factoryFunctionForFilterType[filterType].create;
		...
		function ready(itemsPerPageValue) {
			...
			setTimeout(function() { preSelectFilters(); state.initialised = true }, 1);
		}
		...
		function escapeHashString(string) {
			var escaped = string.replace("Show All", "").replace("--MAX--_", "from_").replace("--MIN--_", "to_").replace("--RANGE--_", "range_");
		...
		function addWordBoundaryMatchForExactMatches(value, id) {
			var nonExactMatch = state.nonExactMatch.indexOf(id+"_"+value) > -1;
